...
REGIME_EXAMINE_THRESHOLDS = {
    ...
    "RANGE_BOUND": {
        ...
        "auto_wait_on_neutral":            False,  # FIX-EXAMINE-03: Don't auto-WAIT
        "allow_proceed_with_caution":      True,    # FIX-EXAMINE-03: Allow reduced-size entries
    },
    "CONSOLIDATION": {
...
# ── FIX-EXAMINE-03: WAIT soft-allow configuration ────────────────────────
# When cross-examiner says WAIT, these are the minimum conditions for
# allowing a reduced-size trade instead of total blockage.
WAIT_SOFT_ALLOW = {
    "min_conviction": 55,         # Minimum conviction to allow under WAIT
    "position_size_multiplier": 0.5,  # 50% position size under WAIT
    "enabled_for_strategies": {   # Which F&O strategies can soft-allow
        "LONG_STRADDLE", "IRON_CONDOR", "STRANGLE",
        "BULL_SPREAD", "BEAR_SPREAD",
    },
}
...
class CrossExaminationResult:
    ...
    # FIX-EXAMINE-03: soft-allow flag — when WAIT but a specific strategy passed
    soft_allow: bool = False  # True allows reduced-size F&O entry under WAIT
    soft_allow_size_mult: float = 0.5  # position size multiplier when soft_allow=True
...
class LLMCrossExaminer(BaseLLMAgent):
    ...
    def _apply_regime_override(
        ...
        """
        FIX-EXAMINE-01 + FIX-EXAMINE-03: Override LLM recommendation when regime logic
        ...
        FIX-EXAMINE-03: In RANGE_BOUND with LOW IV:
          WAIT → soft_allow (50% size, conviction >= 55)
          This enables straddles/condors to execute at reduced risk even when
          the cross-examiner is conservative.
        """
        ...
        # ── FIX-EXAMINE-03: RANGE_BOUND soft-allow ────────────────────
        # In RANGE_BOUND regime, the market is directionless but F&O strategies
        # like straddles and iron condors are still valid. Instead of hard-blocking
        # on WAIT, allow reduced-size entries for vol-selling/buying strategies.
        if regime_name == "RANGE_BOUND" and result.final_recommendation == "WAIT":
            max_conviction = max(max_short_conviction, max_long_conviction)
            iv_regime = market_context.get("iv_regime", "NORMAL")

            # Allow soft entry when:
            # 1. Any directional agent has conviction >= 55
            # 2. IV regime supports a strategy (LOW → straddle, HIGH → condor)
            if max_conviction >= WAIT_SOFT_ALLOW["min_conviction"] or iv_regime in ("LOW", "HIGH"):
                result.soft_allow = True
                result.soft_allow_size_mult = WAIT_SOFT_ALLOW["position_size_multiplier"]
                result.regime_override = True
                result.reasoning = (result.reasoning or "") + (
                    f" [FIX-EXAMINE-03: RANGE_BOUND regime — soft-allowing "
                    f"F&O entries at {WAIT_SOFT_ALLOW['position_size_multiplier']*100:.0f}% "
                    f"size (conviction={max_conviction}, IV={iv_regime})]"
                )
                self.logger.info(
                    f"[FIX-EXAMINE-03] RANGE_BOUND soft-allow: WAIT → allowing "
                    f"reduced-size F&O entries (conviction={max_conviction}, IV={iv_regime})"
                )
                return result

        # ── Original FIX-EXAMINE-01 logic for directional regimes ─────
