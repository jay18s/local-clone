...
class FyersFetcher:
    ...
    def fetch_market_data(self) -> Dict[str, Any]:
        ...
        _usd_inr_source = "YFINANCE"  # default — will be overwritten if futures fallback fires
        if usd_inr == 0.0:
            try:
                ...
                # FIX-DEC-ROLLOVER: when month=12, 12%12=0 → _months[0]="JAN" (correct),
                # but the year suffix must advance to next year for the Jan contract.
                if _today.month == 12:
                    _nxt_mon  = "JAN"
                    _nxt_yr2  = str(_today.year + 1)[-2:]
                    _nxt_sym  = f"NSE:USDINR{_nxt_yr2}{_nxt_mon}FUT"
                else:
                    _nxt_mon  = _months[_today.month % 12]
                    _nxt_sym  = f"NSE:USDINR{_yr2}{_nxt_mon}FUT"
                _fx_syms   = ",".join([
                ...
                if _fx_resp.get("s") == "ok":
                    for _item in _fx_resp.get("d", []):
                        ...
                        if 60 < _ltp < 120:
                            ...
                            usd_inr = _ltp
                            _usd_inr_source = "FUTURES"  # tag so market_impact_monitor can suppress EXTREME
                            logger.info(f"USD/INR (Fyers FX {_sym}): ₹{usd_inr:.2f} [futures price, source=FUTURES]")
                            break
        ...
        return {
            ...
            "usd_inr":             usd_inr,
            "usd_inr_source":      _usd_inr_source,   # "YFINANCE" or "FUTURES" — used by market_impact_monitor to suppress false EXTREME
        }
