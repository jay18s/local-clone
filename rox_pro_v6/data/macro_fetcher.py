...
class MacroFetcher:
    ...
    def fetch(self) -> Dict[str, Any]:
        ...
        # FIX-WEEKEND-01: On non-trading days (weekends / NSE holidays), NSE API
        # returns zeros for FII/DII flows and yfinance INR=X is stale.  Reuse the
        # last cached trading-day data rather than overwriting it with phantom zeros.
        _is_weekend = today.weekday() >= 5  # Saturday=5, Sunday=6
        if _is_weekend and self._cache is not None:
            logger.info(
                f"MacroFetcher: non-trading day ({today.strftime('%A')}) — "
                f"reusing last cached data from {self._cache_date} to avoid zero FII/DII flows."
            )
            return self._cache
