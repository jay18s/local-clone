...
# ── Virtual Execution Bridge (Self-Evaluating Live Simulator) ─────────────
try:
    from execution.virtual_execution_bridge import VirtualExecutionBridge
    from execution.virtual_broker import get_virtual_broker
    from execution.self_improvement import get_self_improvement_engine
    VIRTUAL_BROKER_AVAILABLE = True
except Exception as _vb_err:
    VIRTUAL_BROKER_AVAILABLE = False
    _vb_err_str = str(_vb_err)
...
class DailyTradingPlan:
    ...
    # v6.1 — Virtual Execution Engine report (self-evaluating live simulator)
    virtual_execution_report: str = ""  # Formatted report string
...
class LeadCoordinator:
    ...
    def __init__(self, config: SystemConfig = None, portfolio_value: float = 1_000_000):
        ...
        if V6_MODULES_AVAILABLE:
            ...
            # FIX-CALIBRATOR-PERSIST: Load saved weights immediately so prior learning survives restart
            self.adaptive_calibrator.load()
    ...
    def generate_trading_plan(
        ...
        self._shadow_scan_active     = False   # suppresses duplicate log lines in shadow equity scan
        self._regime_last_logged_date = None   # prevents multiple log_session writes per day
            ...
            try:
                ...
                if examination.agents_to_upweight or examination.agents_to_downweight:
                    ...
                    for aname in examination.agents_to_downweight:
                        if aname in self.agents:
                            # FIX-PHOENIX-WARMUP: Skip weight cuts for non-voting agents
                            # (baseline_weight == 0.0).  PHOENIX is a non-voter — cutting
                            # its weight is a no-op mathematically but wastes scorecard
                            # bandwidth and produces misleading "weight cut 60%" log lines.
                            # More broadly: any agent with baseline_weight=0 should never
                            # have its weight managed by the consensus examiner.
                            _agent_obj = self.agents[aname]
                            if getattr(_agent_obj, "baseline_weight", None) == 0.0:
                                self.logger.debug(
                                    f"[EXAMINE-GATE] Skipping weight cut for non-voter {aname} "
                                    f"(baseline_weight=0.0) — scorecard penalty has no effect."
                                )
                                continue
        ...
        if self.llm_planner is not None:
            try:
                ...
                try:
                    # FIX-SHADOW-LOG: Set flag so _analyze_stock suppresses [RULE-VALIDATE]
                    # log lines during this shadow scan — they already fired in the primary
                    # scan and duplicate entries in the log are confusing.
                    self._shadow_scan_active = True
                    _all_eq = [
                        self._analyze_stock(s, market_data, portfolio_status)
                        for s in watchlist
                    ]
                    self._shadow_scan_active = False
                    ...
                    self._shadow_scan_active = False  # always reset on exception too
                ...
                _plan_ctx = {
                    ...
                    "vix":                market_data.get("india_vix", 15),  # for watchlist regime filter
    ...
    def _tier_11_detect_regime(self, data) -> Tuple[MarketRegime, float]:
        ...
        if self.regime_arbiter is not None and _rule_regime_str is not None and _llm_regime_str is not None:
            ...
            # FIX-REGIME-MAP: Added MILD_BEAR, BULL, MILD_BULL, BEAR, CORRECTION which
            # were missing — any unlisted key fell back to CONSOLIDATION, causing the
            # plan header to show CONSOLIDATION while the arbiter correctly said MILD_BEAR.
            _regime_map = {
                ...
                "VOLATILE":     MarketRegime.CONSOLIDATION,  # map volatile to consolidation
            }
            ...
            if self.regime_accuracy_tracker is not None:
                try:
                    ...
                    # FIX-WEEKEND-LOG: Never log a regime session on weekends — the data
                    # is stale Friday OHLC and creates phantom accuracy records.
                    # Also deduplicate: only log once per calendar day (first cycle).
                    _is_weekend = _today.weekday() >= 5
                    _last_logged = getattr(self, "_regime_last_logged_date", None)
                    _already_logged_today = (_last_logged == _today)
                    if not _is_weekend and not _already_logged_today:
                        # FIX-OHLC-EXTRACT: nifty_open/high/low live in data["nifty_intraday"]
                        # not at the top level. data.get("nifty_open") always returns 0,
                        # making close_pct=0 every session → MILD_BEAR/CAUTIOUS labels never fire.
                        _ni = data.get("nifty_intraday", {})
                        _nifty_price = data.get("nifty_price", 0)
                        _nifty_open  = _ni.get("open",  _nifty_price) or _nifty_price
                        _nifty_high  = _ni.get("high",  _nifty_price) or _nifty_price
                        _nifty_low   = _ni.get("low",   _nifty_price) or _nifty_price
                        self.regime_accuracy_tracker.log_session(
                            rule_regime=_rule_regime_str,
                            rule_confidence=_rule_confidence,
                            llm_regime=_llm_regime_str,
                            llm_confidence=_llm_confidence,
                            nifty_open=_nifty_open,
                            nifty_close=_nifty_price,
                            nifty_high=_nifty_high,
                            nifty_low=_nifty_low,
                            vix_open=data.get("india_vix", 15),
                            vix_close=data.get("india_vix", 15),
                        )
                        self._regime_last_logged_date = _today
                    elif _is_weekend:
                        self.logger.debug(
                            "Regime session skipped: weekend (stale data — no log written)"
                        )
                except Exception:
                    pass
    ...
    def _run_all_agents(self, data, regime):
        ...
        for name, agent in self.agents.items():
            if name in ("NOCTURNAL", "PHOENIX"):
                # NOCTURNAL: runs separately via pre_market_routine
                # PHOENIX: runs in its own dedicated block after consensus (non-voter,
                #          needs full market_data not the agent-specific mapping subset,
                #          and has its own _sessions_run counter that must not double-fire)
                continue
            try:
    ...
    def _tier_2_calculate_consensus(self) -> ConsensusResult:
        ...
        # FIX-CONSENSUS-01: Lowered thresholds so that a single high-conviction directional
        # agent (weight ~0.15-0.16, conviction 0.63) can contribute ~0.10 net_score and
        # clear WEAK.  Old ±0.25 required 3+ agents to agree — mathematically unreachable
        # with 7 neutral agents and only 2 active voters.
        if net > 0.12:
            direction = TradeDirection.LONG
            if net > 0.40:   strength = "STRONG"
            elif net > 0.25: strength = "MODERATE"
            else:            strength = "WEAK"
        elif net < -0.12:
        ...
        _trace_logger.info(
            ...
            f"net_score={consensus_result.net_score + 0.0:.3f} | "
    ...
    def _analyze_stock(self, stock, market_data, portfolio_status):
        ...
        if entry_setup.get("direction") == TradeDirection.NEUTRAL:
            ...
            # FIX-ATR-QUALITY: If ATR equals the default fallback (sp*0.02), it means
            # no real OHLCV history was available.  A fallback ATR produces structurally
            # invalid setups — e.g. HCLTECH R:R=0.04 because the SMA50 target was only
            # 0.5% above entry while the stop was 2% ATR away.  Reject the setup rather
            # than pass a data-quality-failure to the validator.
            _atr_is_default_fallback = (atr == sp * 0.02)
            if _atr_is_default_fallback:
                self.logger.warning(
                    f"[SETUP-GATE] {entry_setup.get('symbol','?')} ATR=sp×0.02 "
                    f"(default fallback — no OHLCV history). Rejecting setup: DATA_QUALITY_FAIL."
                )
                return None
            ...
            # FIX-RR-02: Added MILD_BEAR — arbiter frequently outputs MILD_BEAR but it was
            # missing from this list, causing all setups in MILD_BEAR to be evaluated at the
            # full 1.5 min_rr and rejected.
            if _regime_str in ("CONSOLIDATION", "RANGE_BOUND", "CAUTIOUS", "CORRECTION", "MILD_BEAR"):
            ...
            if vdir == TradeDirection.LONG:
                ...
                risk = sp - stop
                # FIX-STRUCTURAL-FILTER: Only use structural levels that are at least
                # min_rr × risk above entry.  Previously the nearest SMA could be just
                # 0.5% above spot, creating a near-zero R:R that wins over the ATR fallback
                # target and then gets rejected downstream — wasting a validator call.
                _min_target_dist = min_rr * risk if risk > 0 else atr * min_rr
                structural = [x for x in [sma50,sma200,round(sp*1.05/50)*50]
                              if x > sp and (x - sp) >= _min_target_dist]
                ...
                risk = stop - sp
                # FIX-STRUCTURAL-FILTER (SHORT): Only use structural levels that are at
                # least min_rr × risk below entry. Mirrors the LONG path fix.
                _min_target_dist_short = min_rr * risk if risk > 0 else atr * min_rr
                structural = [x for x in [sma50,sma200,round(sp*0.95/50)*50]
                              if x < sp and (sp - x) >= _min_target_dist_short]
        ...
        if _entry_price > 0 and _target_1 > 0 and _stop_loss > 0:
            ...
            if _setup_direction == TradeDirection.LONG:
                ...
                if _target_1 >= _entry_price:
                    if not getattr(self, "_shadow_scan_active", False):
                        self.logger.warning(
        ...
        if self.rule_validator:
            try:
                ...
                # FIX-SHADOW-LOG: Only log and record rejection on primary scan, not shadow scan
                _in_shadow = getattr(self, "_shadow_scan_active", False)
                if not _in_shadow:
                    self.logger.info(f'[RULE-VALIDATE] {stock} → passed={rule_result.passed} score={rule_result.score:.1f} | {rule_result.reason}')
                if not rule_result.passed:
                    if not _in_shadow:
                        # FIX-REJECT-LOG: Wire into RejectionLogger so daily summary counts it
                        try:
                            from utils.rejection_logger import log_rejection as _log_rej
                            _log_rej(
                                module="RULE_VALIDATOR",
                                symbol=stock,
                                strategy=entry_setup.get("direction", TradeDirection.NEUTRAL).value,
                                reason=rule_result.reason,
                                threshold=f"score>={self.rule_validator.min_rr_ratio}",
                                actual=f"score={rule_result.score:.1f} rr={rule_input.get('rr_ratio',0):.2f}",
                                action="REJECTED",
                                conviction=50,
                            )
                        except Exception:
    ...
    def v6_on_trade_close(self, symbol: str, direction: str, pnl: float,
        ...
        Args:
            ...
            is_shadow: FIX-SHADOW-01 — True for paper/shadow trades (cross-examiner
                WAIT outcomes). Shadow trades are weighted 0.3× in PatternMemory to
                prevent phantom SL-hits from corrupting the learning signal.
        """
        # 1. CircuitBreakerV2
        ...
        if self.adaptive_calibrator is not None:
            try:
                ...
                # FIX-CALIBRATOR-PERSIST: Persist updated weights to disk after every trade
                self.adaptive_calibrator.save()
...
class UnifiedCoordinator:
    ...
    def __init__(self, config: Optional[SystemConfig] = None, portfolio_value: float = 1_000_000):
        ...
        # ── Virtual Execution Bridge — Self-Evaluating Live Trade Simulator ──
        self._veb = None
        self._veb_cycle_count = 0
        if VIRTUAL_BROKER_AVAILABLE:
            try:
                self._veb = VirtualExecutionBridge(initial_capital=portfolio_value)
                self.logger.info(
                    "[VEB] ✅ Virtual Execution Bridge ACTIVE — "
                    "F&O predictions tracked as live orders"
                )
            except Exception as _veb_init_err:
                self.logger.warning(f"[VEB] Init failed (non-critical): {_veb_init_err}")
        else:
            self.logger.info("[VEB] Virtual broker unavailable — skipping")
    ...
    def generate_trading_plan(
        ...
        if _exam_rec_unified == "AVOID":
            ...
            try:
                ...
                # Annotate fno_suggestions with examiner verdict for VEB
                if plan.fno_suggestions is not None:
                    plan.fno_suggestions._examiner_verdict = _exam_rec_unified
                    plan.fno_suggestions._examiner_size_mult = (
                        0.50 if _exam_rec_unified == "REDUCE_SIZE" else 1.0
                    )
        ...
        # ── VIRTUAL EXECUTION ENGINE — Self-Evaluating Live Trade Tracker ────
        # Processes every cycle: MTM open positions → auto-exits → execute new
        # suggestions → self-improvement calibration → append report to plan.
        if self._veb is not None:
            try:
                self._veb_cycle_count += 1
                _iv_regime = "LOW"
                _vix_val = float(market_data.get("india_vix", market_data.get("vix", 17.0)))
                if _vix_val >= 25: _iv_regime = "EXTREME"
                elif _vix_val >= 18: _iv_regime = "HIGH"
                elif _vix_val >= 14: _iv_regime = "MODERATE"

                _regime_str = plan.market_regime.value if plan.market_regime else "UNKNOWN"
                _regime_conf = float(
                    getattr(self.lead, "_last_v6_regime_confidence", None)
                    or getattr(plan, "regime_confidence", 68.0)
                )
                _consensus_str = str(plan.consensus.strength) if plan.consensus else "NO_CONSENSUS"

                # Detect holiday/weekend for theta decay + self-review
                from datetime import date as _date
                _today = _date.today()
                _is_holiday = _today.weekday() >= 5  # Sat/Sun
                # Also treat as holiday if market_data has a session flag
                _is_holiday = _is_holiday or bool(market_data.get("is_market_holiday", False))

                _veb_report = self._veb.process_cycle(
                    fno_suggestions=plan.fno_suggestions,
                    market_data=market_data,
                    regime=_regime_str,
                    regime_confidence=_regime_conf,
                    agent_consensus=_consensus_str,
                    cycle_number=self._veb_cycle_count,
                    iv_regime=_iv_regime,
                    is_market_holiday=_is_holiday,
                )
                plan.virtual_execution_report = self._veb.format_execution_report(_veb_report)
                _veb_live_n   = len(_veb_report.get("new_live_executions", []))
                _veb_shadow_n = len(_veb_report.get("new_shadow_executions", []))
                _veb_exits_n  = len(_veb_report.get("exits", []))
                _veb_pf       = _veb_report.get("portfolio", {})
                self.logger.info(
                    f"[VEB] Cycle {self._veb_cycle_count} | "
                    f"live={_veb_live_n} | shadow={_veb_shadow_n} | "
                    f"exits={_veb_exits_n} | "
                    f"open_live={_veb_pf.get('open_positions', 0)} | "
                    f"shadow_open={len(_veb_report.get('open_shadow_positions', []))} | "
                    f"realised=₹{_veb_pf.get('total_realised_pnl', 0):+,.0f}"
                )
            except Exception as _veb_cycle_err:
                self.logger.warning(f"[VEB] Cycle error (non-critical): {_veb_cycle_err}")
    ...
    def format_report(self, plan: DailyTradingPlan) -> str:
        ...
        lines.append(f"\nCONSENSUS: {plan.consensus.direction.value} "
                     f"({plan.consensus.strength}) | net_score={plan.consensus.net_score + 0.0:.3f}")
        ...
        if _ex:
            ...
            if pqa:
                # FIX-PHOENIX-DISPLAY: Filter out non-voting agents (baseline_weight=0)
                # from the Poor-Quality display — their weight cut was skipped in the
                # coordinator loop, so showing them here is misleading.
                lc_agents = getattr(lc, "agents", {})
                _effective_pqa = [
                    a for a in pqa
                    if getattr(lc_agents.get(a), "baseline_weight", 1.0) > 0.0
                ]
                if _effective_pqa:
                    lines.append(f"  ⚠ Poor-Quality: {', '.join(_effective_pqa)} (weight cut 60% — defective reasoning)")
