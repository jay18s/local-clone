...
class ROXUnifiedEngine:
    ...
    def __init__(
        ...
        # FIX-NEWS-BLOCK-01: Non-blocking news fetch state.
        # Instead of future.result(timeout=30) which can stall the 60s main loop,
        # we fire-and-forget the coroutine and check completion next cycle.
        self._news_fetch_future = None   # pending concurrent.futures.Future or None
        self._news_stale_cycles = 0      # how many cycles since last completed fetch
    ...
    def _trading_cycle(self):
        ...
        # FIX-NEWS-BLOCK-01: Non-blocking news fetch.
        # Old code: future.result(timeout=30) could stall the main loop for up to 30s.
        # New code: submit the coroutine in the background; check for completion each
        # cycle.  If a fetch completes, reset stale counter.  If stale for >2 cycles,
        # submit a new fetch (rather than waiting for the blocked one indefinitely).
        # This ensures the 60s main loop never blocks on OpenRouter network latency.
        if self._news_fetch_future is not None and self._news_fetch_future.done():
            try:
                self._news_fetch_future.result()   # surface exceptions to the log
                logger.debug("News fetch completed (non-blocking).")
            except Exception as _ne:
                logger.debug(f"News fetch completed with error: {_ne}")
            self._news_fetch_future = None
            self._news_stale_cycles = 0

        if not self.news_context._current_news or self._news_stale_cycles > 2:
            if self._news_fetch_future is None or self._news_fetch_future.done():
                # No pending fetch — submit a new one (fire-and-forget)
                try:
                    self._news_fetch_future = asyncio.run_coroutine_threadsafe(
                        self.news_context.update(), self._news_loop
                    )
                    logger.debug("News fetch submitted (non-blocking).")
                    self._news_stale_cycles = 0
                except Exception as _ne:
                    logger.debug(f"News context submit failed: {_ne}")
            else:
                # Fetch is still in-flight from a previous cycle
                self._news_stale_cycles += 1
                logger.debug(
                    f"News fetch still in-flight (cycle {self._news_stale_cycles}/2). "
                    f"Proceeding with cached context."
                )
        ...
        # ── Virtual Execution Engine report ─────────────────────────────────
        _veb_rpt = getattr(plan, "virtual_execution_report", "")
        if _veb_rpt:
            try:
                print(_veb_rpt)
            except UnicodeEncodeError:
                import sys
                sys.stdout.buffer.write(_veb_rpt.encode("utf-8"))
