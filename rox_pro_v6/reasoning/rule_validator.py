...
class RuleBasedValidator:
    ...
    def validate(
        ...
        if regime_str in ("CONSOLIDATION", "RANGE_BOUND", "CAUTIOUS", "CORRECTION", "MILD_BEAR"):
        ...
        # FIX-META-REC-02: In MILD_BEAR + VIX 16–20, hard-reject ALL LONG equity setups.
        # Historical data: 4 LONG rejections in MILD_BEAR were all correct calls.
        # Only SHORT setups are viable in confirmed MILD_BEAR regime.
        if regime:
            regime_dir = regime.get("regime", "").upper()
            vix_now = float(market_data.get("india_vix", market_data.get("vix", 0)) or 0)
            ...
            _is_mild_bear = regime_dir in ("MILD_BEAR", "CAUTIOUS")
            _vix_elevated = 16 <= vix_now <= 22 if vix_now > 0 else False

            if regime_dir in BEARISH_REGIMES and direction == "LONG":
                if _is_mild_bear and _vix_elevated:
                    # Hard reject — audit meta-recommendation: no LONG in MILD_BEAR+VIX16-22
                    reasons_fail.append(
                        f"HARD REJECT: LONG in {regime_dir} regime (VIX={vix_now:.1f}) — "
                        f"only SHORT setups accepted in MILD_BEAR+elevated VIX"
                    )
                    score -= 40  # ensures passed=False regardless of other scores
                else:
