...
class AdaptiveConfidenceCalibrator:
    ...
    # ── FIX-CALIBRATOR-PERSIST: Persistence across restarts ──────────────────
    # Previously AdaptiveCalibrator was purely in-memory — all 30+ trades of
    # signal-quality learning was lost on every engine restart.  These two
    # methods save/load weights and recent history to a JSON file so the
    # Bayesian priors accumulate across sessions.

    _PERSIST_PATH = "data/calibration/adaptive_weights.json"

    def save(self, path: Optional[str] = None) -> bool:
        """Persist current weights and recent history to disk."""
        import os
        fpath = path or self._PERSIST_PATH
        try:
            ...
            payload = {
                ...
                "history": [
                    ...
                    for r in self.history[-self.window_size:]  # keep last window only
                ],
            ...
            logger.info(
                f"AdaptiveCalibrator: saved {len(self.weights)} weights + "
                f"{len(payload['history'])} history records → {fpath}"
            )
            return True
        except Exception as e:
            logger.warning(f"AdaptiveCalibrator: save failed: {e}")
            return False

    def load(self, path: Optional[str] = None) -> bool:
        """Load persisted weights and history from disk."""
        fpath = path or self._PERSIST_PATH
        try:
            with open(fpath) as f:
                payload = json.load(f)
            loaded_weights = payload.get("weights", {})
            # Only load weights for signals we still track
            for k in self.weights:
                if k in loaded_weights:
                    self.weights[k] = float(loaded_weights[k])
            self._normalize()
            # Restore history
            ...
            logger.info(
                f"AdaptiveCalibrator: loaded {len(loaded_weights)} weights + "
                f"{len(self.history)} history records from {fpath} "
                f"(saved {payload.get('saved_at', '?')})"
            )
            return True
        except FileNotFoundError:
            logger.info(f"AdaptiveCalibrator: no saved state at {fpath} — starting fresh")
            return False
        except Exception as e:
            logger.warning(f"AdaptiveCalibrator: load failed: {e}")
            return False
