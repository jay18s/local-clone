...
class PatternMemoryBank:
    ...
    def _update_outcome_v6(
        ...
        is_shadow: bool = False,  # FIX-SHADOW-01: flag shadow/paper trades
    ) -> None:
        """
        ...
        Args:
            ...
            is_shadow: FIX-SHADOW-01 — True for paper/shadow trades.
                Shadow trades are conditional simulations (cross-examiner said
                WAIT) — they were never genuinely executed and their SL hits do
                not represent true market tests.  Shadow outcomes are recorded
                with a reduced weight (0.3×) so they don't corrupt the pattern
                accuracy signal equally with real fills.
        """
        try:
            conn = duckdb.connect(self.db_path)

            # FIX-SHADOW-01: Weight shadow outcomes at 30% of a real trade outcome.
            # This preserves the directional signal (pattern fired → outcome) while
            # preventing the bulk of phantom SL-hits from dominating the accuracy calc.
            _shadow_weight = 0.3 if is_shadow else 1.0
            _raw_accuracy  = 1.0 if actual_outcome == "WIN" else 0.0  # will check predicted below
            # We store: prediction_accuracy = weight × raw_accuracy, so that aggregate
            # AVG(prediction_accuracy) naturally discounts shadow entries.
            ...
            conn.execute(
                ...
                (actual_outcome, actual_pnl, hold_period_minutes,
                 actual_outcome, _shadow_weight, match_id),
            )
            ...
            logger.info(
                ...
                f"{'[SHADOW weight=0.3]' if is_shadow else '[LIVE weight=1.0]'}"
            )
