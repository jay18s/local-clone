...
class SystemConfig:
    ...
    def __post_init__(self):
        if not self.agents:
            self.agents = {
                # FIX-META-REC-01 & 04: Applied audit weight adjustments as new baselines.
                # Rationale: 30-trade / 16.7% win rate analysis showed ORION's entry quality
                # is the primary failure mode (R:R near-zero setups, counter-trend entries).
                # SENTINEL correctly called SHORT in MILD_BEAR but lacked weight to overcome
                # KAIRO's LONG vote.  VESPER (macro/flow) should bias entries toward
                # macro-confirmed setups only.
                # ORION: 0.20 → 0.17 (−0.03) | SENTINEL: 0.15 → 0.17 (+0.02) | VESPER: 0.18 → 0.19 (+0.01)
                "ORION":    AgentConfig("ORION",    "Technical Analysis",    0.17),
                "VESPER":   AgentConfig("VESPER",   "Flow Analysis",         0.19),
                "KAIRO":    AgentConfig("KAIRO",    "Sentiment Analysis",    0.12),
                "SENTINEL": AgentConfig("SENTINEL", "Derivatives Analysis",  0.17),
