...
PORTFOLIO_RISK_PCT    = 0.045   # 4.5% per F&O trade on ₹10L portfolio (default)
...
# FIX-DYNAMIC-RISK: Conviction-based risk scaling
# High conviction (>65%) → allow up to 5.5% risk
# Medium conviction (55-65%) → standard 4.5%
# Low conviction (<55%) → cap at 3.5%
RISK_PCT_BY_CONVICTION = {
    "high":   0.055,   # conviction > 65
    "medium": 0.045,   # 55 < conviction <= 65
    "low":    0.035,   # conviction <= 55
}
CONVICTION_HIGH_THRESHOLD = 65
CONVICTION_LOW_THRESHOLD  = 55

def get_dynamic_risk_pct(conviction: int) -> float:
    """Return risk percentage scaled by conviction level."""
    if conviction > CONVICTION_HIGH_THRESHOLD:
        return RISK_PCT_BY_CONVICTION["high"]
    elif conviction > CONVICTION_LOW_THRESHOLD:
        return RISK_PCT_BY_CONVICTION["medium"]
    else:
        return RISK_PCT_BY_CONVICTION["low"]
...
class ChecklistResult:
    ...
    def display_lines(self) -> List[str]:
        checks = [
            ...
            ("Portfolio risk (dynamic by conviction)", self.risk_ok, self.risk_note),
...
def run_checklist(
    ...
    skip_liquidity_check: bool = False,
    index_name: str = "",
    conviction: int = 50,  # FIX-DYNAMIC-RISK: conviction for risk scaling
) -> ChecklistResult:
    """
    ...
    # FIX-DYNAMIC-RISK: Scale risk by conviction
    _risk_pct = get_dynamic_risk_pct(conviction)
    max_risk      = portfolio_value * _risk_pct
    cl.risk_ok    = max_loss_per_lot <= max_risk
    cl.risk_note  = f"max_loss=Rs{max_loss_per_lot:,.0f} limit=Rs{max_risk:,.0f} ({_risk_pct*100:.1f}%/conv={conviction})"
    if not cl.risk_ok:
        cl.risk_note += " BREACH"
    ...
    def advise(
        ...
        for idx_name, spec in INDEX_SPECS.items():
            try:
                ...
                # FIX-META-REC-05: In MILD_BEAR + LOW IV, cap straddle max_loss at ₹27,000.
                # In MILD_BEAR, one-directional moves pin the straddle and both BANKNIFTY
                # and SENSEX straddles hit SL the same day.  The tighter cap prevents
                # oversized losses in a regime that structurally disadvantages long vol.
                _is_mild_bear_regime = str(market_regime).upper() in ("MILD_BEAR", "CAUTIOUS", "CORRECTION")
                _is_low_iv = iv_regime_idx == "LOW"
                _is_straddle = getattr(suggestion, "strategy", "") == "LONG_STRADDLE"
                _ml_cap = 27_000 if (_is_mild_bear_regime and _is_low_iv and _is_straddle) else None
                if _ml_cap and suggestion.max_loss_per_lot > _ml_cap:
                    logger.info(
                        f"[META-REC-05] {idx_name} LONG_STRADDLE max_loss "
                        f"Rs{suggestion.max_loss_per_lot:,.0f} capped to Rs{_ml_cap:,} "
                        f"(MILD_BEAR + LOW IV — one-directional pin risk)"
                    )
                    suggestion.max_loss_per_lot = _ml_cap
                cl = run_checklist(
                    ...
                    index_name        = idx_name,
                    conviction        = consensus_conviction,  # FIX-DYNAMIC-RISK
                )
        ...
        # ── FIX 8: Correlation Risk Check — prevent correlated index exposure ──
        try:
            ...
            for sug in output.suggestions:
                idx = getattr(sug, "index", "UNKNOWN")
                existing_indices = [getattr(s, "index", "") for s in checked_suggestions]
                result = checker.check(idx, existing_indices)
                if result.action == "BLOCK":
                    logger.warning(f"[CORRELATION] BLOCKED: {idx} — {result.message}")
                    skipped_corr.append(f"{idx}: {result.message}")
                elif result.action == "REDUCE_CAP":
                    logger.info(f"[CORRELATION] REDUCE_CAP: {idx} — {result.message}")
                    checked_suggestions.append(sug)
                else:
                    checked_suggestions.append(sug)
            if skipped_corr:
                output.skipped.extend(skipped_corr)
            output.suggestions = checked_suggestions
        except ImportError:
            pass
        except Exception as _corr_err:
            logger.debug(f"Correlation check skipped: {_corr_err}")
