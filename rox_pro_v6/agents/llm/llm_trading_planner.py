...
class LLMTradingPlanner(BaseLLMAgent):
    ...
    def generate_plan(
        ...
        self._last_regime        = str(plan_context.get("regime", "")).upper()
        self._last_vix           = float(plan_context.get("vix", 0) or 0)
        ...
        try:
            ...
            fno_trades = [
                FnoReadyTrade(
                    ...
                    # FIX-CONVICTION-CLAMP: LLM occasionally outputs -1 for conviction
                    # (meaning "no conviction") which displays as "-1%". Clamp to [0, 100].
                    confidence           = max(0, min(100, int(t.get("confidence", 0)))),
                    status               = (
            ...
            # FIX-PLAN-06: Regime-direction guard on equity watchlist.
            # rule_validator hard-rejects LONG setups in MILD_BEAR+VIX16-22 during
            # _analyze_stock, but the LLM trading planner generates the watchlist
            # independently and can still output LONG ideas in a bearish regime.
            # Mirror the same logic here to filter the watchlist before it reaches
            # the operator.
            _regime_now = getattr(self, "_last_regime", "")
            _vix_now    = float(getattr(self, "_last_vix", 0) or 0)
            _BEARISH_REGIMES_WL = {"BEAR", "MILD_BEAR", "CAUTIOUS", "CORRECTION", "BEARISH"}
            _is_bearish_regime  = _regime_now in _BEARISH_REGIMES_WL
            _is_vix_elevated    = 16 <= _vix_now <= 22 if _vix_now > 0 else False
            if _is_bearish_regime and _is_vix_elevated:
                _filtered_wl = []
                for w in watchlist_raw:
                    _dir = str(w.get("direction", "")).upper()
                    if _dir == "LONG":
                        self.logger.info(
                            f"[PLAN-WL-FILTER] Removed {w.get('stock','')} LONG from watchlist "
                            f"— HARD REJECT in {_regime_now} regime (VIX={_vix_now:.1f}). "
                            f"Only SHORT setups accepted."
                        )
                        continue
                    _filtered_wl.append(w)
                watchlist_raw = _filtered_wl or watchlist_raw[:1]  # keep at least 1 if all removed
            ...
            watchlist = [
                EquityWatchItem(
                    ...
                    conviction    = max(0, min(100, int(w.get("conviction", 0)))),
                    sector        = w.get("sector", ""),
                    reason        = w.get("reason", ""),
                )
