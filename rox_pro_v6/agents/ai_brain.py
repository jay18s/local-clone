...
class AIBrain:
    ...
    def _parse(self, raw: str, setups: List[Dict],
        ...
        try:
            ...
            # FIX-BRAIN-REPAIR: Attempt brace-balancing repair before falling back.
            # Mirrors FIX-FNOBRAIN-REPAIR — same truncation root cause.
            data = None
            try:
                import re as _re
                repair = clean
                ...
                logger.warning("Brain JSON truncation repaired — recovered partial response")
            except Exception:
                pass
            if data is None:
                logger.error(f"Brain JSON parse error (unrecoverable): {e} | raw[:300]={raw[:300]}")
